...
function accountReducer(state: AccountState, action: AccountAction): AccountState {
  switch (action.type) {
    ...
    case 'RESET_PENDING_AVATAR':
      return { ...state, pendingAvatar: null, previewUrl: null, uploadProgress: 0 };
    default:
      return state;
  }
}

// --- Hook مخصص لجلب بيانات الحساب ---
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const { data: roleData, error: roleError } = await supabase
          ...
          .eq('user_id', userId)
          .maybeSingle();
        if (!abortController.signal.aborted && !roleError && roleData) {
        ...
        if (!abortController.signal.aborted) {
          console.error(err);
          dispatch({ type: 'SET_ERROR', payload: 'تعذّر تحميل الملف الشخصي. تأكد من اتصالك بالإنترنت.' });
        }
      } finally {
        if (!abortController.signal.aborted) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl">
        <div className="mb-4 flex items-center justify-between">
          ...
          <button onClick={onClose} className="rounded-full p-2 hover:bg-vibes-100">
            <X className="size-5" />
          </button>
        </div>
...
export function AccountPage() {
  ...
  const saveProfile = useCallback(
    async (event: React.FormEvent) => {
      ...
      try {
        ...
        if (pendingAvatar) {
          // رفع الصورة مع تتبع التقدم (اختياري)
        ...
        // تنظيف الصورة القديمة إذا تم تغييرها
        ...
        dispatch({
          type: 'SET_ERROR',
          payload:
            requestError instanceof Error && requestError.message.includes('5MB')
              ? requestError.message
              : 'ما قدرنا نحفظ التعديلات. راجع إعدادات Storage و RLS.',
        });
      } finally {
        dispatch({ type: 'SET_SAVING', payload: false });
      }
    },
    [profile, userId, pendingAvatar, name, marketingConsent, dispatch]
  );
  ...
  const handleSignOut = useCallback(async () => {
    ...
    try {
      ...
      dispatch({ type: 'SET_ERROR', payload: 'تعذّر تسجيل الخروج. جرّب مرة ثانية.' });
    }
  }, [signOut, navigate, dispatch]);

  // إلغاء الصورة المعلقة عند فك التركيب
  useEffect(() => {
    return () => {
      if (previewUrl) URL.revokeObjectURL(previewUrl);
    };
            ...
            <div className="mt-6">
              ...
              <div className="mt-1 h-2 w-full overflow-hidden rounded-full bg-vibes-700">
                <div
                  ...
                  style={{ width: `${Math.min((loyaltyPoints / 100) * 100, 100)}%` }}
                />
              </div>
              <p className="mt-1 text-xs text-vibes-300">
                {loyaltyPoints >= 100 ? '🟢 مؤهل للحصول على مكافأة' : '🔵 تحتاج 100 نقطة للمكافأة'}
              </p>
            </div>
                ...
                {pendingAvatar && (
                  <div className="mt-2 flex items-center gap-2 text-sm">
                    ...
                    {uploadProgress > 0 && uploadProgress < 100 && (
                      <span className="text-vibes-500">{uploadProgress}%</span>
                    )}
                  </div>
                )}
              </div>
        ...
        <section className="mt-8 rounded-3xl border border-vibes-100 bg-white/80 p-5">
          <div className="flex items-start gap-3">
            ...
            <p className="text-sm leading-7 text-vibes-700">
              المعروض هنا من بيانات الحساب يأتي من Supabase فقط. أما الخدمات التي لم تُنشأ جداولها
              ومنطقها بعد، فلا يعرض النظام أرقاماً أو نتائج وهمية لها.
            </p>
          </div>
        </section>
      </div>
