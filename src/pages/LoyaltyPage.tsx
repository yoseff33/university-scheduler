...
import { ArrowRight, Award, Coffee, History } from 'lucide-react'
    ...
    const fetchLoyalty = async () => {
      try {
        ...
        const { data: profile, error: profileError } = await supabase
          .from('profiles')
          .select('loyalty_points, membership_number, name')
                  ...
                  return (
                    <div key={t.id} className="flex items-center justify-between border-b border-vibes-100 pb-2 last:border-0">
                      <div className="flex items-center gap-3">
                        <span className={`text-sm font-bold ${isEarn ? 'text-emerald-600' : 'text-red-500'}`}>
                          {isEarn ? '+' : '-'}{t.points}
                        </span>
                        <span className="text-sm text-vibes-600">{t.description}</span>
                      </div>
